# ==============================================================================
# BHIFR Empirical Study: Ireland Weather Data
# 
# Description: 
#   Application of the BHIFR model to daily temperature interval curves.
#   Includes data processing, 5-fold cross-validation, final model fitting 
#   (with two chains), and generation of publication-ready tables and figures.
#
# Outputs: 
#   - Table 1: Cross-validation IMSPE comparison
#   - Table 2: Posterior summary of scalar parameters
#   - Figures 3-5: Functional estimates and convergence diagnostics
# ==============================================================================
...
double calc_marginal_loglik_lambda_dense(const vec& lambda, const mat& X_L, const mat& X_U,
    ...
    // Kronecker Product Acceleration
    mat PTP = P.t() * P;
...
// Main MCMC Algorithm (Heteroscedastic Model)
// [[Rcpp::export]]
...
if(!file.exists(zip_path)) {
  stop("ERROR: Data file not found. Please place 'aggregated_hourly_data.zip' in the '../data/' directory.")
}

raw_data <- read.csv(unz(zip_path, csv_name))

data_cleaned <- raw_data %>%
  mutate(datetime = ymd_hms(date, tz = "UTC"), day = as.Date(datetime), hour = hour(datetime)) %>%
  dplyr::select(day, hour, st_name, temp, rhum, msl, wdsp) %>% 
  filter(!is.na(temp) & !is.na(rhum) & !is.na(msl) & !is.na(wdsp))

response_data <- data_cleaned %>% group_by(day, hour) %>%
  summarise(Temp_L = min(temp), Temp_U = max(temp), .groups = 'drop') %>%
  mutate(Temp_L=ifelse(is.infinite(Temp_L),NA,Temp_L), Temp_U=ifelse(is.infinite(Temp_U),NA,Temp_U))

predictor_data <- data_cleaned %>% group_by(day) %>%
  summarise(rhum_L=min(rhum), rhum_U=max(rhum), msl_L=min(msl), msl_U=max(msl), wdsp_L=min(wdsp), wdsp_U=max(wdsp), .groups='drop')

model_data <- left_join(response_data, predictor_data, by = "day") %>%
  filter(year(day) >= 2018 & year(day) <= 2022) %>% na.omit()
...
K_folds <- 5
set.seed(123) # Seed for CV splits
...
# Basic Functional Linear Model fit
...
# Predict function for benchmark
...
# F-CRM: Functional Center-Range Method
...
# F-MinMax: Functional Min-Max Method
...
print(
  kbl(cv_summary, format="latex", booktabs=T, digits=3, caption="5-Fold Cross-Validation IMSPE") %>% 
    kable_styling(latex_options="striped")
)

# ==============================================================================
# PART 4: Final Model Fit (Two Parallel Chains)
# ==============================================================================
...
# Thinning (Keep every 5th sample for efficient storage and plotting)
thin_idx <- seq(1, nrow(fit_chain1$beta), by=5)

# Combine results (Chain 1 used for point estimates)
...
plot_emp_curve <- function(df, ylab_expr) {
  ggplot(df, aes(x=t, y=Mean, color=Type, fill=Type)) +
    geom_line(size=0.8) +
    geom_ribbon(aes(ymin=Lower, ymax=Upper), alpha=0.2, color=NA) +
    scale_color_npg() +
    scale_fill_npg() +
    labs(x=expression(italic(t)), y=ylab_expr) + 
    theme_elegant_journal_legend()
}

p3a <- plot_emp_curve(df_mu, expression(mu(t))) + ggtitle("(a)")
p3b <- plot_emp_curve(df_sig, expression(sigma(t))) + ggtitle("(b)")
...
var_exprs <- c(
  "Humidity" = "beta[1](t)~~(Humidity)",
  "Pressure" = "beta[2](t)~~(Pressure)",
  "Wind Speed" = "beta[3](t)~~(Wind~Speed)"
)
