...
# ==============================================================================
# PART 2: Load Data (Updated for Zip)
# ==============================================================================
zip_path <- file.path("..", "data", "aggregated_hourly_data.zip")
csv_name <- "aggregated_hourly_data.csv"

if(!file.exists(zip_path)) stop("Data file not found at: ", zip_path)
...
cat("Data Loaded. N =", nrow(Y_L_final), "\n")

# Use a random subset for sensitivity analysis if N is too large (Optional, but full data is better)
# For full paper, use FULL data.
# 5-fold CV split (using Fold 1 as Test set for IMSPE calculation)
set.seed(123)
