...
# --- Custom Color Palette (Matched to Fig 1) ---
custom_model_cols <- c("Homo" = "#84D8D7", "Hetero" = "#B8AEEA")
...
res_mu_h <- get_mu_ci(fit_h_demo, 1) 
res_mu_ht <- get_mu_ci(fit_ht_demo, 1) 
...
plot_comparison_curve_with_legend <- function(df, y_col_true, y_col_h, y_col_ht, 
                                              ...
                                              ylab_expr, title_text) {
  ggplot(df, aes(x=t)) +
    
    # Ribbons (Alpha increased slightly for visibility with lighter colors)
    geom_ribbon(aes(ymin=.data[[l_col_h]], ymax=.data[[u_col_h]], fill="Homo"), alpha=0.4) +
    geom_ribbon(aes(ymin=.data[[l_col_ht]], ymax=.data[[u_col_ht]], fill="Hetero"), alpha=0.4) +
    ...
    scale_fill_manual(name="Model", values=custom_model_cols) +
    scale_color_manual(name="Model", values=custom_model_cols) +
...
p2b <- plot_comparison_curve_with_legend(
  ...
  "(b) Variance Function Estimation"
)
