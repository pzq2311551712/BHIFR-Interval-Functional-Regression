...
results <- foreach(i=1:nrow(sim_grid), .combine=bind_rows, .packages=pkgs, .options.snow=list(progress=progress)) %dopar% {
  ...
  res_rows[[length(res_rows)+1]] <- data.frame(method="BHIFR", param="mu_L", imse=calc_imse(train_dat$true_params$mu_L, muL), est_error=NA)
  res_rows[[length(res_rows)+1]] <- data.frame(method="BHIFR", param="mu_U", imse=calc_imse(train_dat$true_params$mu_U, muU), est_error=NA)
  
  for(j in 1:3) {
    bL <- get_s(30+(j-1)*30+1); bU <- get_s(30+(j-1)*30+16)
    res_rows[[length(res_rows)+1]] <- data.frame(method="BHIFR", param=paste0("beta_",j,"L"), imse=calc_imse(train_dat$true_params$Beta_L[[j]], bL), est_error=NA)
    res_rows[[length(res_rows)+1]] <- data.frame(method="BHIFR", param=paste0("beta_",j,"U"), imse=calc_imse(train_dat$true_params$Beta_U[[j]], bU), est_error=NA)
  }
  
  # Prediction (Directly from C++ output)
  res_rows[[length(res_rows)+1]] <- data.frame(method="BHIFR", param="IMSPE_L", imse=calc_imse(test_dat$Y_L, fit$pred_L_mean), est_error=NA)
  res_rows[[length(res_rows)+1]] <- data.frame(method="BHIFR", param="IMSPE_U", imse=calc_imse(test_dat$Y_U, fit$pred_U_mean), est_error=NA)
...
tab2_res <- results %>% 
  ...
  dplyr::select(-Mean, -SD) %>% # Fixed: Explicitly use dplyr::select
  pivot_wider(names_from=method, values_from=Val) %>%
  arrange(param, n, C)
...
print(kbl(tab3_res, format="latex", booktabs=T, digits=6, caption="Table 3: Parameter Estimation (Part 2: Beta2 & Beta3)") %>% 
        kable_styling(latex_options=c("striped", "scale_down")))
...
custom_cols <- c("Lower Bound" = "#84D8D7", "Upper Bound" = "#B8AEEA")

# Minimalist theme (Plain text, no bold)
theme_clean_journal <- function() {
  theme_classic(base_family = "sans", base_size = 10) + 
    theme(
      text = element_text(color = "black"),
      # Changed face="bold" to "plain" as requested
      ...
      strip.text = element_text(face = "plain", size = 10, margin = margin(b = 5)),
...
# Regenerate demo data for plotting
set.seed(999)
...
# Plotting Function
plot_curve_with_legend <- function(df, ylab_expr, title_text, show_legend = FALSE) {
  p <- ggplot(df, aes(x=t)) +
    # CI Ribbons
    geom_ribbon(aes(ymin=LL, ymax=UL, fill="Lower Bound"), alpha=0.4) + # Increased alpha
    geom_ribbon(aes(ymin=LU, ymax=UU, fill="Upper Bound"), alpha=0.4) +
    ...
    # Scales (Using Custom Colors)
    scale_color_manual(values = custom_cols) +
    scale_fill_manual(values = custom_cols) +
    scale_linetype_manual(values = c("True Value"="dashed", "Estimate"="solid")) +
    
    # Labels (Adding the title here)
    labs(y=ylab_expr, x=expression(italic(t)), title=title_text) +
    theme_clean_journal()
...
p_combined_final <- (
  (plot_curve_with_legend(df_mu, expression(mu(t)), "(a) Intercept Function", TRUE)) + 
    (plot_curve_with_legend(df_b1, expression(beta[1](t)), "(b) Deterministic Coefficient", TRUE))
) / (
  (plot_curve_with_legend(df_b2, expression(beta[2](t)), "(c) Random Coefficient", TRUE)) + 
    (plot_curve_with_legend(df_b3, expression(beta[3](t)), "(d) Zero Effect", TRUE))
) + 
  ...
  guides(
    ...
    color = guide_legend(title = NULL, nrow = 1, order = 2, override.aes = list(linetype = 1, size=2)),
